...
class ImageDecoderThread extends Thread {
    ...
    private GeneralBounds dbTileEnvelope;
    ...
    public void run() {
        ...
        try {
            ...
            if (requestEnvelope.contains(dbTileEnvelope, true)) {
                ...
                GeneralBounds intersectionEnvelope = new GeneralBounds(dbTileEnvelope);
                intersectionEnvelope.intersect(requestEnvelope);

                // x and y refers here to Image coordinates (so y is inverted wrt Envelope)
                int xmin = (int) (Math.round(
                        (intersectionEnvelope.getMinimum(0) - dbTileEnvelope.getMinimum(0)) / levelInfo.getResX()));
                int ymin = (int) (Math.round(
                        (dbTileEnvelope.getMaximum(1) - intersectionEnvelope.getMaximum(1)) / levelInfo.getResY()));
                int xmax = (int) (Math.round(
                        (intersectionEnvelope.getMaximum(0) - dbTileEnvelope.getMinimum(0)) / levelInfo.getResX()));
                int ymax = (int) (Math.round(
                        (dbTileEnvelope.getMaximum(1) - intersectionEnvelope.getMinimum(1)) / levelInfo.getResY()));
                int width = xmax - xmin;
                ...
                if ((width > 0) && (height > 0)) {
                    ...
                    tileQueue.add(new TileQueueElement(location, clippedImage, intersectionEnvelope));
                }
            }
    ...
    /**
     * Fallback Method, in some jre implementations, ImageIO.read(InputStream in) returns null. If this happens, this
     * method is called, which is not so efficient but it works
     */
