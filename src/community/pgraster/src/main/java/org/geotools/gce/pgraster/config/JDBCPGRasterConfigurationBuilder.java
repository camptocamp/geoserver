...
/**
 * A JDBC PGRaster configuration builder class. It allows to configure a PGRaster based ImageMosaicJDBC starting from a
 * configuration bean specifying PG credentials, database, table, coverage name and files extensions
 *
 * <p>The configuration may be specified through an input string with the following format:
 * {@literal pgraster://USER:PASS@HOST:PORT:DATABASE.SCHEMA.TABLE@EPSGCODE}, or through a
 * {@link JDBCPGrasterConfigurationBean} instance.
 *
...
public class JDBCPGRasterConfigurationBuilder {
    ...
    private static void initTemplate() {
        ...
        try {
            TEMPLATE = Resources.readLines(resource, StandardCharsets.UTF_8).stream()
    ...
    /**
     * Prepare the PGRaster configuration steps: - importing tiles into the DB and create metadata table if needed -
     * prepare the URL of the XML configuration file and return that URL.
     *
     * <p>The input url String may represent: - 1) the path of the folder containing tiles to be mosaicked, previously
     * generated with gdal_retile. 2) an inline configuration specification in that form:
     ...
     * @param hints hints containing a {@link JDBCPGrasterConfigurationBean} bean in case the url string doesn't specify
     *     the configuration params inline.
     * @return the URL of the generated XML file containing the ImageMosaicJDBC configuration (mapping + connection +
     *     coverage configs)
     */
    public static URL createConfiguration(final String url, final Hints hints) {
        ...
        try {
            if (url.startsWith("pgraster")) {
                ...
                configUrl = new JDBCPGRasterConfigurationBuilder(config, new URL(fileUrl)).buildConfiguration();
            } else if (hints != null && hints.containsKey(JDBCPGrasterConfigurationBean.CONFIG_KEY)) {
                ...
                if (object instanceof JDBCPGrasterConfigurationBean) {
                    ...
                    configUrl = new JDBCPGRasterConfigurationBuilder(config, new URL(url)).buildConfiguration();
                }
            }
    ...
    /**
     * Extract configuration parameters from the provided string. See
     * {@link JDBCPGRasterConfigurationBuilder#createConfiguration(String, Hints)} for the syntax of this string
     */
    private static JDBCPGrasterConfigurationBean parseConfig(final String pgrasterUrl) {
        if (pgrasterUrl != null && pgrasterUrl.startsWith("pgraster:/")) {
            ...
            final String table =
                    pgrasterUrl.substring(schemaEndIndex + 1, epsgStartIndex != -1 ? epsgStartIndex : tableEndIndex);
    ...
    /**
     * Main mosaic configuration method: given a folder containing several tiles, this static helper method does this: i
     * - import raster tiles into the database ii - create metadata table and put new tables on it iii - prepare
    ...
    /**
     * Check whether the specified mosaic is already available in the Database. In that case I don't need to do the
     * raster tiles import.
     *
    ...
    /**
     * Check whether the main metadata table is present and all the tileTables specified inside it (if any) exists too.
     ...
     */
    private void createMetadataTable(final Connection connection, final String tableName, final String coverageName) {
        ...
        try {
            ...
            if (LOGGER.isLoggable(Level.SEVERE)) {
                LOGGER.severe("Exception occurred while "
