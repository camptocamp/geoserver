...
public class JDBCAccess {
    ...
    public void startTileDecoders(
        ...
        try (Connection con = getConnection();
            ...
            try (ResultSet r = s.executeQuery()) {
                while (r.next()) {
                    ...
                    GeneralBounds tileGeneralEnvelope = new GeneralBounds(tmp);
                    tileGeneralEnvelope.setCoordinateReferenceSystem(requestEnvelope.getCoordinateReferenceSystem());

                    ImageDecoderThread thread = new ImageDecoderThread(
                            tileBytes, "", tileGeneralEnvelope, requestEnvelope, levelInfo, tileQueue);
                    // thread.start();
                    threads.add(thread);
                    pool.execute(thread);
                }
        ...
        if (LOGGER.isLoggable(Level.INFO))
            LOGGER.info("Getting and decoding  "
                    + threads.size()
                    + " Tiles needs "
                    + ((new Date()).getTime() - start.getTime())
                    + " millisecs");
        ...
        try (PreparedStatement s = con.prepareStatement(stmt)) {
            ...
            try (ResultSet res = s.executeQuery()) {
                ...
                while (res.next()) {
                     ...
                     * value.
                     */
                    Number noDataValue = getNoDataValue(
                            imageLevelInfo.getTileTableName(), getConfig().getBlobAttributeNameInTileTable(), con);
    ...
    private Number getNoDataValue(String coverageTableName, String blobAttributeName, Connection con)
            throws SQLException {
     ...
     *
     * <p>Calculating the the extent for each image level (original + pyramids). This calculation is only done if the
     * extent info in the master table is SQL NULL. After calculation the meta table is updated with the result to avoid
    ...
    void calculateExtentsFromDB(String coverageName, Connection con) throws SQLException, IOException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(getConfig().getSqlUpdateMosaicStatement())) {
            ...
            for (ImageLevelInfo li : getLevelInfos()) {
                ...
                String envSelect = "with envelopes as ( select st_envelope("
                        + rasterAttr
                        + " ) as env from "
                        + li.getTileTableName()
     ...
     *
     * <p>Calculating the the resolution for each image level (original + pyramids). This calculation is only done if
     * the resoltion info in the master table is SQL NULL. After calculation the meta table is updated with the result
     * to avoid this operation in the future.
     *
     * @param coverageName The coverage name in the sql meta table
     * @param con JDBC Connection
     */
    void calculateResolutionsFromDB(String coverageName, Connection con) throws SQLException, IOException {
        ...
        try (PreparedStatement stmt = con.prepareStatement(getConfig().getSqlUpdateResStatement())) {
    ...
    private void populateStatementsMap(String coverageName, Connection con) throws SQLException {
        ...
        for (ImageLevelInfo li : getLevelInfos()) {
            ...
            String select = "select (ST_BandMetaData("
                    + getConfig().getBlobAttributeNameInTileTable()
                    + ")).isoutdb "
                    + " from "
                    + li.getTileTableName()
     ...
     */
    protected Polygon polyFromEnvelope(GeneralBounds env) {
