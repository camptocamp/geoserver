...
/**
 * {@link AbstractGridFormat} sublass for controlling {@link PostgisRasterGridCoverage2DReader} creation.
 *
 * <p>As the name says, it handles mosaicing georeferenced images and image pyramids, based on tiles stored in a JDBC
 * database.
 *
 * <p>The stored tiles in the database could have any format jai can decode.
 *
 * <ul>
 *   <li>DefaultParameterDescriptor OUTPUT_TRANSPARENT_COLOR = new DefaultParameterDescriptor("OutputTransparentColor",
 *       Color.class, null, null); The default value is white
 * </ul>
 *
 * @author mcr
 * @since 2.5
...
public class PostgisRasterFormat extends AbstractGridFormat implements Format {
        ...
        try {
            if (source instanceof File) {
                ...
                if (path.contains("pgraster:/")) {
                    ...
                    sourceURL = JDBCPGRasterConfigurationBuilder.createConfiguration(path, (Hints) null);
                } else {
    ...
    private void setInfo() {
        ...
        readParameters = new ParameterGroup(new DefaultParameterDescriptorGroup(
                mInfo,
                new GeneralParameterDescriptor[] {READ_GRIDGEOMETRY2D, OUTPUT_TRANSPARENT_COLOR, BACKGROUND_COLOR}));
