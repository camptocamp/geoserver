...
/**
 * This reader is responsible for providing access to images and image pyramids stored in a JDBC database as
 * Postgis_Raster extension tiles.
 *
 * @author mcr
 * @since 2.5
 */
public class PostgisRasterGridCoverage2DReader extends AbstractGridCoverage2DReader {
    ...
    public PostgisRasterGridCoverage2DReader(Object source, Hints uHints) throws IOException, MalformedURLException {
        this.source = source;
        ...
        if (tempCRS != null) {
            ...
            if (tempcrs == null) {
                ...
                LOGGER.log(
                        Level.WARNING,
                        new StringBuffer("Unable to find a CRS for this coverage, using a default one: ")
                                .append(crs.toWKT())
                                .toString());
            } else {
                crs = tempcrs;
        ...
        this.originalEnvelope = new GeneralBounds(new Rectangle2D.Double(
        ...
        originalGridRange =
                new GridEnvelope2D(new Rectangle((int) Math.round(originalEnvelope.getSpan(0) / highestRes[0]), (int)
    ...
    public GridCoverage2D read(GeneralParameterValue... params) throws IOException {
        ...
        state.setOutputTransparentColor(PostgisRasterFormat.OUTPUT_TRANSPARENT_COLOR.getDefaultValue());
        ...
        if (params != null) {
            for (GeneralParameterValue generalParameterValue : params) {
                ...
                if (param.getDescriptor()
                    ...
                    state.setRequestedEnvelope((GeneralBounds) gg.getEnvelope());
                    ...
                    state.setBackgroundColor((Color) param.getValue());
                        ...
                        .getName()
                        .getCode()
                        .equals(PostgisRasterFormat.OUTPUT_TRANSPARENT_COLOR
                                .getName()
                                .toString())) {
    ...
    private void transformRequestedEnvelope(PostgisRasterReaderState state) throws DataSourceException {
        ...
        try {
            ...
            if (config.getIgnoreAxisOrder() == false) { // check for axis order required
                ...
                if (indexX == indexRequestedY && indexY == indexRequestedX) {
                    ...
                    state.setRequestedEnvelopeTransformed(new GeneralBounds(tmp));
                    state.getRequestedEnvelopeTransformed().setCoordinateReferenceSystem(crs);
            ...
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine(new StringBuffer("Reprojected envelope ")
    ...
    private void expandRequestedEnvelope(PostgisRasterReaderState state) {
        ...
        GeneralBounds ret = state.getRequestedEnvelopeTransformed();
        ...
        GeneralBounds expanded = new GeneralBounds(minXY, maxXY);
    ...
    private GridCoverage2D loadTiles(PostgisRasterReaderState state) throws IOException {
        ...
        if (LOGGER.isLoggable(Level.FINE)) {
            LOGGER.fine(new StringBuffer("Creating mosaic to comply with envelope ")
                    .append(
                    ...
                    .append(crs.toWKT())
                    .append(" dim ")
                    .append((renderedImageRectangle == null) ? " null" : renderedImageRectangle.toString())
                    .toString());
        }
        ...
        if (renderedImageRectangle != null) {
            try {
                ...
                BufferedImage emptyImage = state.getEmptyImage(
                        (int) renderedImageRectangle.getWidth(), (int) renderedImageRectangle.getHeight());
                GeneralBounds envelope = state.getRequestedEnvelope();
        ...
        ImageComposerThread imageComposerThread = new ImageComposerThread(state, config, coverageFactory);
        imageComposerThread.start();

        jdbcAccess.startTileDecoders(
                state.getRequestedEnvelopeTransformedExpanded(), levelInfo, state.getTileQueue(), coverageFactory);
    ...
    private GridCoverage2D transformResult(GridCoverage2D coverage, PostgisRasterReaderState state) {
        ...
        // coverage.show();
        GridCoverage2D result = (GridCoverage2D) Operations.DEFAULT.resample(
                coverage, state.getRequestedEnvelope().getCoordinateReferenceSystem());
        // result.show();
