...
public class ImageComposerThread extends Thread {
    ...
    public ImageComposerThread(PostgisRasterReaderState state, Config config, GridCoverageFactory coverageFactory) {
        this.state = state;
        this.config = config;
    ...
    private Dimension getStartDimension() {
        ...
        int width =
                (int) Math.round(state.getRequestedEnvelopeTransformedExpanded().getSpan(0)
                        / state.getImageLevelInfo().getResX());
        int height =
                (int) Math.round(state.getRequestedEnvelopeTransformedExpanded().getSpan(1)
    ...
    private BufferedImage getStartImage(BufferedImage copyFrom) {
        ...
        SampleModel sm =
                copyFrom.getSampleModel().createCompatibleSampleModel((int) dim.getWidth(), (int) dim.getHeight());
    ...
    private BufferedImage getStartImage(int imageType) {
        ...
        Dimension dim = getStartDimension();
        BufferedImage image = new BufferedImage((int) dim.getWidth(), (int) dim.getHeight(), imageTypeReviewed);
    ...
    public void run() {
        ...
        GeneralBounds rete = state.getRequestedEnvelopeTransformedExpanded();
        ...
        try {
            while ((queueObject = tileQueue.take()).isEndElement() == false) {
                ...
                int posx = (int) (Math.round((queueObject.getEnvelope().getMinimum(0) - rete.getMinimum(0))
                        / state.getImageLevelInfo().getResX()));
                int posy = (int) (Math.round(
                        (rete.getMaximum(1) - queueObject.getEnvelope().getMaximum(1))
                                / state.getImageLevelInfo().getResY()));

                image.getRaster().setRect(posx, posy, queueObject.getTileImage().getRaster());
            }
        } catch (OutOfMemoryError e) {
            LOGGER.warning("Out of memory when trying to render coverage '"
                    + state.getImageLevelInfo().getCoverageName()
                    + "'.");
            LOGGER.warning("Tips: increase memory, add coarser dataset overviews, or do not zoom that out.");
            // return no image
            gridCoverage2D = null;
            return;
        ...
        if (state.isXAxisSwitch()) {
            Rectangle2D tmp =
                    new Rectangle2D.Double(rete.getMinimum(1), rete.getMinimum(0), rete.getSpan(1), rete.getSpan(0));
        ...
        if (state.getOutputTransparentColor() == null) {
            gridCoverage2D = coverageFactory.create(config.getCoverageName(), image, resultEnvelope);
        } else {
            if (LOGGER.isLoggable(Level.FINE)) {
                LOGGER.fine("Support for alpha on final mosaic");
            }
            RenderedImage result = ImageUtilities.maskColor(state.getOutputTransparentColor(), image);
        ...
        switch (dataType) {
            ...
            default:
                throw new IllegalStateException(
                        "Couldn't create DataBuffer for  data type " + dataType + " and " + pixelSize + " pixel size");
        }
        return dataBuffer;
    }
    ...
    protected BufferedImage rescaleImageViaPlanarImage(BufferedImage image) {
        ...
        if (!(scaledImageRaster instanceof WritableRaster)) {
            scaledImageRaster = result.copyData(scaledImageRaster.createCompatibleWritableRaster(rirO));
        }

        BufferedImage scaledImage = new BufferedImage(
                image.getColorModel(), (WritableRaster) scaledImageRaster, image.isAlphaPremultiplied(), null);
        return scaledImage;
    }
}
