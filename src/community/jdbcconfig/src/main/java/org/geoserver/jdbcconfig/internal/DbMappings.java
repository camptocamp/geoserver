...
public class DbMappings {
     ...
     *
     * <p>Populated by {@link #reloadPropertyTypes}, possibly multiple times during initialization to account for race
     * conditions on clustered environments.
     */
    ...
    /**
     * @return {@code true} if the property types shall be reloaded because when creating one, someone else won the
     *     race, and hence the {@link #propertyTypes} map is out of sync with the db.
     */
    private boolean addDirectPropertyTypes(
            final Class<? extends Info> clazz, final NamedParameterJdbcOperations template) {

        log("Creating property mappings for %s", clazz.getName());
        ...
        for (String propertyName : properties) {
            ...
            if (returnType.isPrimitive() || returnType.isEnum() || INDEXABLE_TYPES.contains(returnType)) {
                ...
                PropertyType addedProp = addPropertyType(template, clazz, propertyName, null, false, isText);
                boolean createdBySomeoneElse = null == addedProp;
                needsReloading |= createdBySomeoneElse;
            } else {
                log("Ignoring property %s:%s", propertyName, returnType.getSimpleName());
            }
        }
    ...
    private void addNestedPropertyTypes(
        ...
        for (PropertyTypeDef ptd : nestedPropDefs) {
            ...
            if (targetPropertyOf != null) {
                ...
                Map<String, PropertyType> targetPropertyTypes = this.propertyTypes.get(targetPropId);
                checkState(
    ...
    private PropertyType addPropertyType(
        ...
        log("Checking for %s", propertyName);
        ...
        if (exists == 0) {
            ...
            try {
                ...
                pType = null;
                log("Not adding property type %s.%s as it already exists", infoClazz.getSimpleName(), propertyName);
            }
        } else {
            log("Not adding property type %s.%s as it already exists", infoClazz.getSimpleName(), propertyName);
            pType = null;
    ...
    private void log(String msg, Object... msgArgs) {
        if (LOGGER.isLoggable(Level.FINER)) {
            try {
                ...
                LOGGER.log(Level.WARNING, "log format error", e);
            }
        }
    }
