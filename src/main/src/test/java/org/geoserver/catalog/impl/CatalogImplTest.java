...
public class CatalogImplTest extends GeoServerSystemTestSupport {
    ...
    public void testChangeDataStoreWorkspace_fails_on_no_matching_namespace() throws Exception {
        ...
        IllegalArgumentException expected = assertThrows(IllegalArgumentException.class, () -> catalog.save(ds2));
        assertThat(
                expected.getMessage(),
                containsString(
    ...
    @Test
    public void testSaveDataStoreRollbacksStoreWhenFailsToUpdateResourcesNamespace() throws Exception {
        addFeatureType();
    ...
    public void testSaveDataStoreRollbacksBothStoreAndResources() throws Exception {
        ...
        assertEquals("store's not rolled back before throwing", oldWorkspace, store2.getWorkspace());

        List<ResourceInfo> resources = catalog.getResourcesByStore(store2, ResourceInfo.class);
        assertFalse(resources.isEmpty());
        for (ResourceInfo resource : resources) {
            assertEquals("ft's not rolled back before throwing", oldNamespace, resource.getNamespace());
        }
    }
    ...
    private void verifyNamespaceOfStoreResources(StoreInfo store) {
        ...
        for (ResourceInfo resource : resources) {
            ...
            String msg = format(
                    "resource %s of store %s did not get its namespace updated", resource.getName(), store.getName());
